...
class CalibrateHMD(WindowVR, Sequence):
    ...
    status = dict(
        wait = dict(start_target="target", start_pause="pause"),
    ...
    fixation_time = traits.Float(0.3, desc="Time in seconds to display target before calibration")
    ...
    def init(self):
        ...
        print("Calibration cycle step:", self.calibration_cycle_step)
        print("Number of samples per target:", (self.calibration_time * self.fps) / self.calibration_cycle_step)
    ...
    #### TEST FUNCTIONS ####
    def _test_start_target(self, ts):
    ...
    def _while_target_calibrate(self):
        ...
        if self.cycle_count % self.calibration_cycle_step == 0:
