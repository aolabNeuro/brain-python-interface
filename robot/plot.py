...
def plot(robot, tg, **kwargs): 
    ...
    if isempty(rh) or isempty( get(gcf, 'Children')):
    ...
    if ishold and isempty( findobj(gca, 'Tag', robot.name)):
...
def create_new_robot(robot, opt):
    ...
    if opt.base:
        b = transl(robot.base)
        line('xdata', np.r_[b(1), b(1)], 'ydata', np.r_[b(2), b(2)], 'zdata', np.r_[h.zmin, b(3)], 'LineWidth', 4, 'color', 'red')
    ...
    if opt.wrist:   
        h.x = line('xdata', np.r_[0,0], 'ydata', np.r_[0,0], 'zdata', np.r_[0,0], 'color', 'red')
        h.y = line('xdata', np.r_[0,0], 'ydata', np.r_[0,0], 'zdata', np.r_[0,0], 'color', 'green')
        h.z = line('xdata', np.r_[0,0], 'ydata', np.r_[0,0], 'zdata', np.r_[0,0], 'color', 'blue')
    ...
    for i in range(1, robot.n):
        ...
        if opt.joints:
            ...
            xyz = np.vstack([xc.reshape(-1,1), yc.reshape(-1,1), zc.reshape(-1,1), np.ones(1,2*N+2)])
