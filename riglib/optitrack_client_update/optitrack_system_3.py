...
class OptiTrackData(object):
    ...
    # Class-level attributes required by BMI3D's MultiChanDataSource
    ...
    def __init__(self, server_ip=DEFAULT_OPTITRACK_SERVER_IP, client_ip=DEFAULT_OPTITRACK_CLIENT_IP, 
        ...
        update_freq : float
            Expected frame rate (Hz) - this overrides the class default
        buffer_size : int
            Size of data buffer
        ...
        self.update_freq = update_freq  # Instance attribute can override class attribute
        self.send_data_to_sink_manager = send_data_to_sink_manager
        self.channels = channels or []
        
        # Update the class attribute if a different frequency is specified
        # This ensures BMI3D's source.py uses the correct frequency
        if update_freq != self.__class__.update_freq:
            self.__class__.update_freq = update_freq
