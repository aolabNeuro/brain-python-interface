...
import copy
import time
from riglib.optitrack_client_update import DataDescriptions
from riglib.optitrack_client_update import MoCapData
...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        self.server_ip_address = "128.95.215.191" #"127.0.0.1"

        # Change this value to the IP address of your local network interface
        self.local_ip_address = "128.95.215.213" #"127.0.0.1"

        # This should match the multicast address listed in Motive's streaming settings.
        self.multicast_address = "239.255.42.100"
    ...
    def run( self, thread_option):
        ...
        if thread_option == 'd':
            ...
            if self.command_thread.is_alive():
                print('Command thread is alive, starting data thread')
                self.data_thread.start()
                print('data thread started')

        # Create a separate thread for receiving command packets
        if thread_option == 'c':
            print('starting command thread only')
            self.command_thread.start()
