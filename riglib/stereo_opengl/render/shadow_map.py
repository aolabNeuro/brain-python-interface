...
class ShadowMapper(FBOrender):
    ...
    def generate_shadow_map(self, root, **kwargs):
        ...
        near_plane = 1.0
        far_plane = 1024.0
        light_projection = orthographic(30, 30, near_plane, far_plane)
        light_view = look_at(-30*np.array(self.light_direction[:3]), light_target, light_up)
        ...
        self.draw_to_fbo(self.shadow_map, root, shader="shadow_pass", apply_default=True, 
                         light_space_matrix=light_space_matrix, **kwargs)
        ...
        return self.shadow_map['depth'], light_space_matrix
